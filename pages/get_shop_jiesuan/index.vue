<template>
	<view style="height: 100%">
		...
		<view style="padding: 30rpx; width: 85%; background-color: white"
			class="mx-auto pingfang rounded-xl shadow-xl flex flex-col">
		...
		<view @click="pay" style="width: 85%; background-color: rgb(24, 144, 224); padding: 30rpx; margin-top: 90rpx"
				...
				that.$data.shop_list.forEach(item => {
					uni.request({
						...
						data: {
							...
							custorm_address: that.$data.address[0].address + "_" + that.$data.address[0].phone, //客户地址
							img: item.img, //商品图片
						},
						success(res) {
							if (res.data.code == 1) {
