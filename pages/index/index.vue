    ...
    <view style="height: 100%">
        <view class="flex justify-between items-center" style="margin-top: 79rpx; padding: 10px">
            <view class="" style="font-weight: 900; font-size: 12px; color: rgb(102, 102, 102)">
                <view style="font-size: 14px; color: black; width: 444rpx">Welcome to Second hand trading platform</view>
                <view style="margin-top: 10rpx">欢迎使用二手交易平台</view>
            </view>

            <view
                class="pingfang border text-center flex justify-center items-center"
        ...
        <view class="wrapper">
            <view class="scroll-content">
                <view
                    ...
                    @tap="get_shop_deatiled"
                    :data-id="item.good_id"
										v-if="item.show"
                    style="width: 400rpx; display: inline-block; margin: 10px"
                    v-for="(item, index) in good_list"
                    :key="index"
                >
                    <image style="width: 100%; height: 90px" mode="aspectFill" :src="item.img[0]"></image>

                    <view class="pingfang font-bold flex justify-between" style="padding-left: 10px; padding-right: 10px; padding-top: 10px">
                        <view>{{ item.name }}</view>
                        <image src="/static/images/icon/Path.png" style="width: 12px; height: 15px"></image>
                    </view>

                    <view style="padding: 10px; font-size: 12px; padding-bottom: 0 px" class="pingfang">￥{{item.price}}</view>
      ...
      <view class="w-full pingfang flex  "
            @tap="get_shop_deatiled"
						v-if="item.show"
            :data-id="item.good_id"
...
<!--                <view style="font-size: 12px;padding-left: 10px;">{{item.username}}</view>-->
          ...
          <view class="text-xs" style="width: 60%;overflow: hidden;text-overflow: ellipsis;padding: 10px;">{{item.introduce}}</view>
